...
MIN_STREAMS   = 1                   # floor: large files never drop below this
MAX_STREAMS   = 200                   # ceiling: small files never exceed this
...
async def get_file_size(client: HttpClient, url: str) -> int:
    ...
    try:
        ...
        if resp.content_length > 0:
            logger.info("GET %s → Content-Length: %d bytes", url, resp.content_length)
            return resp.content_length
        logger.warning("GET %s returned no Content-Length", url)
        return 0
    except Exception as e:
...
async def main(
    ...
    try:
        ...
        # Python 3.10 compatible connection wrapper
        async with conn_ctx as client:
            client = cast(HttpClient, client)
            for url in urls:
                # Wrap the pipeline run inside wait_for to enforce the 15s handshake check indirectly,
                # or manage the setup phase natively. Since wait_connected=True is handled inside conn_ctx,
                # wrapping the inner context manager execution ensures compatibility.
